...
trait EvolutionsApi {
   ...
   */
  def scripts(db: String, evolutions: Seq[Evolution], dbConfig: EvolutionsDatasourceConfig): Seq[Script]
   ...
   * @return evolution scripts
   */
  def scripts(db: String, reader: EvolutionsReader, dbConfig: EvolutionsDatasourceConfig): Seq[Script]
   ...
   * @return evolution scripts
   */
  def resetScripts(db: String, dbConfig: EvolutionsDatasourceConfig): Seq[Script]
   ...
   * @param schema The schema where all the play evolution tables are saved in
   */
  def evolve(db: String, scripts: Seq[Script], autocommit: Boolean, dbConfig: EvolutionsDatasourceConfig): Unit
   ...
   * @param schema The schema where all the play evolution tables are saved in
   */
  def resolve(db: String, revision: Int, dbConfig: EvolutionsDatasourceConfig): Unit
}
...
class DefaultEvolutionsApi @Inject() (dbApi: DBApi) extends EvolutionsApi {
  ...
  private def databaseEvolutions(name: String, dbConfig: EvolutionsDatasourceConfig) = new DatabaseEvolutions(dbApi.database(name), dbConfig)

  def scripts(db: String, evolutions: Seq[Evolution], dbConfig: EvolutionsDatasourceConfig) = databaseEvolutions(db, dbConfig).scripts(evolutions)

  def scripts(db: String, reader: EvolutionsReader, dbConfig: EvolutionsDatasourceConfig) = databaseEvolutions(db, dbConfig).scripts(reader)

  def resetScripts(db: String, dbConfig: EvolutionsDatasourceConfig) = databaseEvolutions(db, dbConfig).resetScripts()

  def evolve(db: String, scripts: Seq[Script], autocommit: Boolean, dbConfig: EvolutionsDatasourceConfig) = databaseEvolutions(db, dbConfig).evolve(scripts, autocommit)

  def resolve(db: String, revision: Int, dbConfig: EvolutionsDatasourceConfig) = databaseEvolutions(db, dbConfig).resolve(revision)
}

/**
 * Evolutions for a particular database.
 */
class DatabaseEvolutions(database: Database, dbConfig: EvolutionsDatasourceConfig) {
  ...
  private def databaseEvolutions(): Seq[Evolution] = {
    ...
    try {
      ...
      Collections.unfoldLeft(executeQuery(
        """
            select id, hash, apply_script, revert_script from ${dbConfig.schema}${dbConfig.evolutionsTable} order by id
        """)) { rs =>
        rs.next match {
          case false => None
          case true => {
  ...
  def evolve(scripts: Seq[Script], autocommit: Boolean): Unit = {
    def logBefore(script: Script)(implicit conn: Connection): Unit = {
      script match {
        case UpScript(e) => {
          val ps = prepare("insert into ${schema}${dbConfig.evolutionsTable} (id, hash, applied_at, apply_script, revert_script, state, last_problem) values(?, ?, ?, ?, ?, ?, ?)")
        ...
        case DownScript(e) => {
          execute("update ${schema}${dbConfig.evolutionsTable} set state = 'applying_down' where id = " + e.revision)
        }
      }
    }
    ...
    def logAfter(script: Script)(implicit conn: Connection): Boolean = {
      script match {
        case UpScript(e) => {
          execute("update ${schema}${dbConfig.evolutionsTable} set state = 'applied' where id = " + e.revision)
        }
        case DownScript(e) => {
          execute("delete from ${schema}${dbConfig.evolutionsTable} where id = " + e.revision)
        }
      }
    }

    def updateLastProblem(message: String, revision: Int)(implicit conn: Connection): Boolean = {
      val ps = prepare("update ${schema}${dbConfig.evolutionsTable} set last_problem = ? where id = ?")
  ...
  private def checkEvolutionsState(): Unit = {
    def createPlayEvolutionsTable()(implicit conn: Connection): Unit = {
      try {
        ...
        case NonFatal(ex) => logger.warn("could not create ${schema}${dbConfig.evolutionsTable} table", ex)
      }
    }
    ...
    try {
      val problem = executeQuery("select id, hash, apply_script, revert_script, state, last_problem from ${schema}${dbConfig.evolutionsTable} where state like 'applying_%'")
  ...
  def resolve(revision: Int): Unit = {
    implicit val connection = database.getConnection(autocommit = true)
    try {
      execute("update ${schema}${dbConfig.evolutionsTable} set state = 'applied' where state = 'applying_up' and id = " + revision)
      execute("delete from ${schema}${dbConfig.evolutionsTable} where state = 'applying_down' and id = " + revision);
    } finally {
      connection.close()
    }
  }
  ...
  private def applySchema(sql: String): String = {
    sql.replaceAll("\\$\\{schema}", Option(dbConfig.schema).filter(_.trim.nonEmpty).map(_.trim + ".").getOrElse(""))
  }
...
private object DefaultEvolutionsApi {
  ...
  val CreatePlayEvolutionsSql =
    """
      create table ${schema}${dbConfig.evolutionsTable} (
  ...
  val CreatePlayEvolutionsOracleSql =
    """
      CREATE TABLE ${schema}${dbConfig.evolutionsTable} (
          ...
          state Varchar2(255),
          last_problem clob,
          CONSTRAINT ${dbConfig.evolutionsTable}_pk PRIMARY KEY (id)
      )
    """
